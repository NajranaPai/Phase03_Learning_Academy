 ...
 */
@WebServlet("/TeacherSubjectMain")
public class TeacherSubjectAssignmetServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TeacherSubjectAssignmetServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<TeacherSubject> ts=session.createQuery(hql).list();
		if(ts.size()>=1)
		{
			...
			out.print("</tr>");
			for(TeacherSubject t:ts)
			{
				out.print("<tr>");
				out.print("<td align=center>"+ t.getSrno() +"</td>");
				hql="From Teacher where Teacher_Id=:Teacher_Id";
				List<Teacher> tname=session.createQuery(hql).setParameter("Teacher_Id",t.getTeacher_Id()).list();
				...
				List<Course> cname=session.createQuery(hql).setParameter("Course_Id", t.getCourse_Id()).list();
				for(Course c: cname)
				{
					out.print("<td align=center>"+ c.getCourse_Name()  +"</td>");
				}
				hql="From Subject Where Subject_Id=:Subject_Id";
				List<Subject> subject=session.createQuery(hql).setParameter("Subject_Id",t.getSubject_Id()).list();
				for(Subject s: subject)
				{
					out.print("<td align=center>"+ s.getSubject_Name()  +"</td>");
				}
				out.print("<td align=center width=100px><a href=EditTeacherSubject?tsid="+ t.getSrno()+ ">Edit</a></td>");
				out.print("<td align=center width=100px><a href=DeleteTeacherSubject?tsid="+t.getSrno() + ">Delete</a></td>");
				out.print("</tr>");
			}
			out.print("</table>");
			out.print("<br><a href=AssignTeacherSubject>Are You want to Assign Subject to teacher?</a>");
		...
		else
		{
			...
			out.print("<font style=color:red> No Subject Assign to teacher Details Found </font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
			out.print("<a href=AssignTeacherSubject>Are You want to Assign Subject to teacher?</a></div>");
