...
public class CourseUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    out.print("</style></head><body bgcolor=#7575FF><form action=EditCourse method=post>");
		...
		if(course.size()>=1)
		{
			...
			for(Course c:course)
			{
				if(c.getCourse_Id().equalsIgnoreCase(cid))
				{
				 out.print("<tr>");
				 out.print("<td align=center width=150px><input type=text class=txt name=\"cid\" style=\"height:30px; width:150px;\" value=\""+c.getCourse_Id()+"\"></td>");
				 out.print("<td align=center width=150px><input type=text class=txt name=\"cname\" style=\"height:30px; width:250px;\" value=\""+c.getCourse_Name() +"\"></td>");
