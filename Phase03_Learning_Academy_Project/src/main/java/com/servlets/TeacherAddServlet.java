...
public class TeacherAddServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println(".main {height:400px;   width:100%;margin:auto;background-color:white;text-align:Center; font-family:cursive; font-size:120%;}");
	    out.print(".p6{height: 30px;   width: 95%; margin:auto;   background-color:white;   text-align:Center; font-family:cursive; font-size:120%;}");
	    ...
	    out.print("</style></head><body bgcolor=#7575FF><form action=AddTeacher method=post>");
	   ...
	   // out.print("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp");
		...
		out.print("<tr>");
		int n=teacher.size()+1;
		String tid;
		if(n<10) { tid="T0"+n;} else { tid="T"+n;}
		out.print("<td align=center width=100px><input type=text class=txt name=\"tid\" style=\"height:30px; width:100px;\" value="+tid +"></td>");
		out.print("<td align=center width=200px><input type=text class=txt name=\"fname\" style=\"height:30px; width:200px;\"/></td>");
		out.print("<td align=center width=200px><input type=text class=txt name=\"lname\" style=\"height:30px; width:200px;\"/></td>");
		out.print("<td align=center width=200px><input type=text class=txt name=\"qualification\" style=\"height:30px; width:200px;\"/></td>");
		out.print("<td align=center width=250px><textarea class=txt name=\"address\" style=\"height:30px; width:250px;\"></textarea></td>");
		out.print("<td align=center width=200px><input class=txt type=text name=\"phoneno\" style=\"height:30px; width:200px;\"/></td>");
		
		out.print("<td colspan=2 align=center><input type=submit value=Add class=b></td></tr>");
		out.print("</table>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
			Session session = sessionFactory.openSession();
			String hql="from Teacher where Teacher_Id=:Teacher_Id or (First_Name=:First_Name and Last_Name=:Last_Name and Qualification=:Qualification and Address=:Address and PhoneNo=:PhoneNo)";
			List<Teacher> teacher=session.createQuery(hql).setParameter("Teacher_Id",tid).setParameter("First_Name",fname).setParameter("Last_Name",lname).setParameter("Qualification",qualification).setParameter("Address",address).setParameter("PhoneNo",phoneno).list();
			if(teacher.size()==0)
			{
