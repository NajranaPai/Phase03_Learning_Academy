...
public class StudentAddServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    out.print("</style></head><body bgcolor=#7575FF><form action=AddStudent method=post>");
		...
		String Studid;
		if(n<10) { Studid="Stud0"+n;} else { Studid="Stud"+n;}
		out.print("<td align=center width=100px><input type=text class=txt name=\"stid\" style=\"height:30px; width:100px;\" value="+Studid+"></td>");
		out.print("<td align=center width=200px><input type=text class=txt name=\"sfname\" style=\"height:30px; width:200px;\"/></td>");
		out.print("<td align=center width=200px><input type=text class=txt name=\"slname\" style=\"height:30px; width:200px;\"/></td>");
		out.print("<td align=center width=200px><input type=text class=txt name=\"seducation\" style=\"height:30px; width:200px;\"/></td>");
		out.print("<td align=center width=250px><textarea class=txt name=\"saddress\" style=\"height:30px; width:250px;\"></textarea></td>");
		out.print("<td align=center width=200px><input class=txt type=text name=\"sphoneno\" style=\"height:30px; width:200px;\"/></td>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			SessionFactory sessionFactory = HibernateUtil.buildSessionFactory();
			Session session = sessionFactory.openSession();
			String hql="from Student where Student_Id=:Student_Id or (First_Name=:First_Name and Last_Name=:Last_Name and Education=:Education and Address=:Address and PhoneNo=:PhoneNo)";
			List<Student> student=session.createQuery(hql).setParameter("Student_Id",sid).setParameter("First_Name",fname).setParameter("Last_Name",lname).setParameter("Education",education).setParameter("Address",address).setParameter("PhoneNo",phoneno).list();
			if(student.size()==0)
			{
