...
public class CourseDeleteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Course course=session.load(Course.class,request.getParameter("cid"));
		session.delete(course);
		
		String hql="Delete from Subject where Course_Id=:Course_Id";
		Query q=session.createQuery(hql).setParameter("Course_Id",request.getParameter("cid"));
		int n=q.executeUpdate();
		tx.commit();
		
		tx=session.beginTransaction();
		hql="Delete from TeacherSubject where Course_Id=:Course_Id";
		q=session.createQuery(hql).setParameter("Course_Id",request.getParameter("cid"));
		n=q.executeUpdate();
		tx.commit();
		
		tx=session.beginTransaction();
		hql="Delete from StudentCourse where Course_Id=:Course_Id";
		q=session.createQuery(hql).setParameter("Course_Id",request.getParameter("cid"));
		n=q.executeUpdate();
		tx.commit();
